...
from wtforms.validators import DataRequired, Length, Email, EqualTo, Optional, Regext
...
class UploadAvatarForm(FlaskForm):
    image = FileFiled('Upload', validators=[
        FileRequired(),
        FielAllowed(['jpg', 'png'], 'The file format should be .jpg or .png.')
    ])
    submit = SubmitField()
...
class ChangePasswordForm(FlaskForm):
    old_password = PasswordField('Old Password', validators=[DataRequired()])
    password = PasswordField('New Password', validators=[DataRequired(), Length(8, 255), EqualTo(password2)])
    password2 = PasswordField('Confirm Password', validators=[DataRequired()])
    submit = SubmitField()
